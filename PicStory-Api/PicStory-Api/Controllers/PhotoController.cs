...
namespace PicStory_Api.Controllers
{
    ...
    public class PhotoController : ControllerBase
    {
        ...
        [HttpGet("user/{userId}")]
        public async Task<ActionResult> GetPhotosByUserId(int userId)
        {
            var photos = await _photoServices.GetAllAsync();
            var userPhotos = photos.Where(p => p.UserId == userId);

            var photosDto = _mapper.Map<IEnumerable<PhotoDTO>>(userPhotos);
        ...
        public async Task<ActionResult> GetPhotosByAlbumId(int albumId)
        {
            var photos = await _photoServices.GetPhotosByAlbumIdAsync(albumId);
